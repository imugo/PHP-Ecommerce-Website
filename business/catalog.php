<?php
// Business tier class for reading catalog information
class Catalog {
	// Define products display  options
	public static $mProductDisplayOptions = array('Default', 'On Catalog', 'On Department', 'On Both');
	...
	// Calculate how many pages of products could be filled by the number of products returned by the $countSql query
	private static function HowManyPages($countSql, $countSqlParams) {
		...
		// Very if we have query results in cache
		if (isset($_SESSION['last_count_hash'])	&& isset($_SESSION['how_many_pages']) && 
				$_SESSION['last_count_hash'] == $queryHashCode) {
					...
					// Save the query and the last count in the session
					$_SESSION['last_count_hash'] = $queryHashCode;
	...
	// Retrieves list of products that belongs to a category
	public static function GetProductsInCategory($categoryId, $pageNo, &$rHowManyPages) {
		// Query that returns the amount of products in a category
	...
	public static function GetProductsOnDepartment($departmentId, $pageNo, &$rHowManyPages) {
		// Query that returns the number of products in the departmwnt page
		...
		// Retrieve the list of products
		$sql = 'CALL catalog_get_products_on_department(:department_id, :short_product_description_length, :products_per_page,
														:start_item)';
		
		// Build the parameters array
		$params = array(':department_id' => $departmentId, ':short_product_description_length' => SHORT_PRODUCT_DESCRIPTION_LENGTH,
							':products_per_page' => PRODUCTS_PER_PAGE, ':start_item' => $start_item);
	...
	public static function GetProductsOnCatalog($pageNo, &$rHowManyPages) {
		...
		// Retrieve the list of products
		$sql = 'CALL catalog_get_products_on_catalog(:short_product_description_length, :products_per_page,
														:start_item)';
		
		// Build the parameters array
		$params = array(':short_product_description_length' => SHORT_PRODUCT_DESCRIPTION_LENGTH,
							':products_per_page' => PRODUCTS_PER_PAGE, ':start_item' => $start_item);
	...
	// Retrieve the list of new products
	public static function GetNewProductsOnCatalog() {
		// Build the query
		$sql = 'CALL catalog_get_new_products_on_catalog()';
	...
	public static function Search($searchString, $allWords, $pageNo, &$rHowManyPages) {
		...
		// Return void if the search result is void
		...
		// If there aren't any accepted words return the search result
		if (count($search_result['accepted_words']) == 0)
			return $search_result;
			
		// Build search string from accepted word list
		$search_string = '';
		...
		$params = array(':search_string'=>$search_string, ':all_words'=>$allWords, ':short_product_description_length'=>SHORT_PRODUCT_DESCRIPTION_LENGTH, ':products_per_page'=>PRODUCTS_PER_PAGE, ':start_item'=>$start_item);
	...
	// Retrieve all departments with their descriptions
	...
	public static function DeleteAttributeValue($attributeValueId) {
		// Build the sql query
		$sql = 'CALL catalog_delete_attribute(:attribute_value_id)';
	...
	public static function GetRecommendations($productId) {
		// Build the SQL query
		$sql = 'CALL catalog_get_recommendations(:product_id)';
